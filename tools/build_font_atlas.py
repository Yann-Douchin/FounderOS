...
def build(dirpath, *, use_typographic_metrics=False):
    ...
    if use_typographic_metrics:
        ascent, descent = info["typoAscent"], info["typoDescent"]
    else:
...
for f in FONTS:
    d = "d_" + f
    if os.path.isdir(d):
        # LanaPixel reports a larger FreeType ascent than the 11 px line height
        # used by the BUSY Bar. Its typographic metrics match the firmware.
        atlas[f] = build(d, use_typographic_metrics=(f == "global"))
