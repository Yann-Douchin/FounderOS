...
class ContentIconTests(unittest.TestCase):
    def test_semantics_take_priority_over_source(self) -> None:
        cases = (
            (Event(source="gmail", title="Production bloquée", kind="blocker"), "alert"),
            (Event(source="linear", title="Réunion avec un investisseur"), "calendar"),
            (Event(source="linear", title="Répondre à l’e-mail du client"), "mail"),
            (Event(source="gmail", title="Mention Slack à traiter"), "chat"),
            (Event(source="slack", title="Corriger le bug de l’API"), "code"),
            (Event(source="gmail", title="Croissance des ventes"), "trend"),
            (Event(source="slack", title="Approuver la proposition"), "decision"),
        )
    ...
    def test_source_and_action_fallbacks_cover_normalized_events(self) -> None:
        cases = (
            (Event(source="calendar", title="Point quotidien"), "calendar"),
            (Event(source="gmail", title="Nouveau sujet"), "mail"),
            (Event(source="slack", title="Nouveau sujet"), "chat"),
            (Event(source="linear", title="Nouveau sujet"), "task"),
            (Event(source="demo", title="Nouveau sujet", action_required=True), "decision"),
            (Event(source="demo", title="Nouveau sujet"), "focus"),
        )
        for event, expected in cases:
    ...
    def test_frame_index_loops(self) -> None:
        ...
        self.assertEqual(first_name, "task")
