...
The installation creates two private user LaunchAgents. `com.founderos.busybar-emulator` binds the emulator to `127.0.0.1` and stores emulator state under the private FounderOS application-state directory, never in the checkout. `com.founderos.runtime` starts FounderOS after login and restarts it after unsuccessful exits. Their plists contain executable paths and non-secret environment settings only. A first installation requires the independent live connector preflight. An in-place upgrade may instead use a fresh healthy heartbeat whose PID matches the currently supervised process, which avoids launching a competing network poll from the terminal. The installer snapshots both prior definitions, retries transient `launchctl bootstrap` failures with bounded exponential backoff across the macOS throttle interval, and never force-kills the process that `RunAtLoad` just started. It then requires the replacement process to publish its own fresh `running` heartbeat with a matching launchd PID, healthy display, healthy connectors, and healthy automations. If readiness fails, it restores the exact prior plists and loaded states. An old heartbeat or a live but degraded process cannot hide a failed start.
